...
//    ercode = /*** TODO ***/ ;	   // Update the inode
  ...
  // NOTE: just the direct pointers, no support for the indirect
   /*** TODO ***/
   unsigned char data[DISK_BLOCK_SIZE];
   memset(data, 0, DISK_BLOCK_SIZE);
   for (int i = 0; i < DPOINTERS_PER_INODE; i++) {
       if (ino.dptr[i] != 0) {
           disk_ops.write(ino.dptr[i], data, 1);
           bmap_ops.set(DATA_BMAP, (ino.dptr[i] - super_ops.getStartDtArea(&ffs_IMsb.sb) + 1), FREE);
           ino.dptr[i] = 0;
       }
   }
 ...
 /*** TODO ***/
  ercode = bmap_ops.set(INODE_BMAP, inoNbr, FREE);
  if (ercode < 0) return ercode; // This would be a bug!
...
static int ffs_file_write(const int fd, const void *buffer,\
  ...
  /*** TODO ***/
    lDskBlkToWrite = L2P(openFT[fd].IMino.ino.dptr[fileBlkNbr]);
//  memcpy(buffer, openFT[fd].IMino.ino, )

  // Write the Block!
//  ercode=disk_ops.write( /*** TODO ***/, buffer, CLUSTER_SIZE );
  ercode=disk_ops.write(lDskBlkToWrite, buffer, CLUSTER_SIZE );
  if (ercode < 0) return ercode; // This would be a bug!
  ...
  /*** TODO ***/
  if (newCluster) {
      ercode = bmap_ops.set(DATA_BMAP, cluster2set, 1);
      if (ercode < 0) return ercode; // This would be a bug!
  }
