...
static void super_create(struct super *sb, unsigned int nblocks, \
    ...
    sb->sizeInBmap = 1;
    sb->startInArea = (sb->startInBmap + sb->sizeInBmap);     //2
    sb->sizeInArea = sizeInArea;
    sb->ninodes = sizeInArea * INODES_PER_BLK;
    sb->startDtBmap = (sb->startInArea + sizeInArea);
    unsigned int total_nclusters = (((nblocks / clusterSize) + DISK_BLOCK_SIZE - 1) / DISK_BLOCK_SIZE);
    unsigned int data_nclusters = (((nblocks - sb->startDtBmap - total_nclusters) + clusterSize - 1) / clusterSize);
    //pode-se afinar...

//    sb->sizeDtBmap = (((nblocks - sb->startDtBmap) + DISK_BLOCK_SIZE - 1) / DISK_BLOCK_SIZE;
    sb->sizeDtBmap = (data_nclusters + DISK_BLOCK_SIZE - 1) / DISK_BLOCK_SIZE;
    sb->clusterSize = clusterSize;
    sb->startDtArea = (sb->startDtBmap + sb->sizeDtBmap);
    sb->nclusters = data_nclusters;
...
//    memcpy( /*** TODO ***/ );
    memcpy(&sb_u.sb, sb, sizeof(struct super));
...
/**** These should be in the mount() library call, not here...
  bmap_ops.mount(&imSB->sb);	// in-mem bmap infos: start, size, n.entries
  ffs_file_ops.mount();		// clear the open files table
***/
