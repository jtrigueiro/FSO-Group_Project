...
//    if (numinode >= /*** TODO ***/ 0)
    if (numinode < 0 && numinode >= ((ffs_IMsb.sb.ninodes) / INODE_SIZE))
        return -EINVAL;
...
static int inode_update(const unsigned int numinode, const struct inode *in) {
    ...
    /*** TODO ***/
    ercode = disk_ops.read((block + INODE_OFFSET(&ffs_IMsb.sb)), i_b.data, 1);
    if (ercode < 0) return ercode;

    // merge inode into block
    /*** TODO ***/
    memcpy(&i_b.ino[offset], in, sizeof(struct inode));

    // write inode block to disk
    /*** TODO ***/
    ercode = disk_ops.write((block + INODE_OFFSET(&ffs_IMsb.sb)), i_b.data, 1);
    if (ercode < 0) return ercode;
...
//  if ( /*** TODO ***/0 < 0) return -EINVAL;
    if (numinode < 0 && numinode >= (ffs_IMsb.sb.ninodes)) return -EINVAL;

    // read inode block from disk into local mem
    /*** TODO ***/
    inode_location(numinode, &block, &offset);
    ercode = disk_ops.read((block + INODE_OFFSET(&ffs_IMsb.sb)), i_b.data, 1);
    if (ercode < 0) return ercode;
...
//    unsigned int end = /*** TODO ***/ 0;
    unsigned int end = start + super_ops.getSizeInArea(sb);
