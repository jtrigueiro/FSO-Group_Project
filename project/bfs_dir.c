...
int bfs_dir_mkEmptydir(unsigned int root, unsigned int pIno) {
  ...
  union u_dir dirBuf[dbs/szE]; memset(dirBuf, 0, dbs);

  inode= ffs_file_ops.create(DIR_FILE);
  if (inode < 0) return inode;

  if ( (root) && (inode > 0) ) return -EINVAL; // BUG, should be zero

  ercode= ffs_file_ops.open(inode);
  if (ercode < 0) return ercode;
  int fd= ercode;

  strcpy(dirBuf[0].dirE.name, "."); dirBuf[0].dirE.inoNbr= inode;
  strcpy(dirBuf[1].dirE.name, ".."); dirBuf[0].dirE.inoNbr= pIno;
...
int bfs_dir_opendirI(struct IMdirectory *cwd, unsigned int inoNbr) {
  ...
  // Just checking :-)
  if (strcmp(cwd->dirBuf[0].dirE.name,".")) return -EINVAL;
  if (strcmp(cwd->dirBuf[1].dirE.name,"..")) return -EINVAL;
...
int bfs_dir_mount(struct IMdirectory *cwd) {
  ...
  strcpy(cwd->dirent.name, "/"); cwd->dirent.inoNbr= 0;
...
int  bfs_dir_readdir(struct IMdirectory *cwd, struct dentry *dirent) {
  ...
  if (cwd->dirPos < 0) return (cwd->dirPos); // Already at end

  /*** TODO ***/
  memcpy(dirent, &(cwd->dirBuf[cwd->dirPos].buf), sizeof(struct dentry));
  cwd->dirPos++;

  if (cwd->dirPos == (DATA_BLOCK_SIZE / sizeof(struct dentry)))
      cwd->dirPos = -1;
...
int matchAndFree(struct IMdirectory *cwd, int op, char *name) {
  int ercode, pos= -1;
  struct dentry dirent; memset(&dirent, 0, sizeof(struct dentry));
...
int bfs_dir_create(struct IMdirectory *cwd, char *name, unsigned int type) {
  ...
  if (matchAndFree(cwd, MATCH, name) > 0) return -EEXIST;
  if ( (free= matchAndFree(cwd, FREE,  name)) < 0 ) return -ENOSPC;

  if (type == REG_FILE) {
    ercode= ffs_file_ops.create(type);
    if (ercode < 0) return ercode;
  } else { // create file and block with empty dir, etc. 0=NOT ROOT DIR
    ercode= bfs_dir_mkEmptydir(0, cwd->dirent.inoNbr);
    if (ercode < 0) return ercode;
  }

  // write the dirent to the current dir buffer
  dirent.inoNbr= ercode; strcpy(dirent.name, name);
...
int bfs_dir_cd(struct IMdirectory *cwd, char *name) {
  ...
  if ( strcmp(name, "/") != 0 )
    if ( (pos=matchAndFree(cwd, MATCH, name)) < 0 ) return -ENOENT;
